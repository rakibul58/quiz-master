...
const NavBar = () => {
  ...
  return (
    <nav className="bg-green-500 flex w-full justify-between py-2 items-center">
      <h1 className="lg:ml-28 text-2xl ml-2 font-bold text-white">Quiz Master</h1>
      <div onClick={() => setOpen(!open)} className="h-6 w-6 md:hidden mr-2 text-white">
        {open ? <XMarkIcon /> : <Bars3Icon />}
      </div>
      <div
        className={`bg-green-500 text-lg font-semibold text-white p-4 lg:mr-28 absolute flex flex-col justify-center md:flex-row w-full md:w-fit md:static duration-500 ease-in ${
